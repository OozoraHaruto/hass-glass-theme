/*
 * glass-refraction.js -- compatibility support for older Liquid Glass YAML
 * generated by hass-glass-theme.
 *
 ...
 *
 * INSTALL (optional compatibility support)
 *
 *   1. Copy this file to `<config>/www/glass-refraction.js`.
 *   2. Add it under `frontend.extra_module_url` in configuration.yaml.
 *   3. Restart Home Assistant and select older Liquid Glass YAML that defines
 *      all three activation variables below.
 *
 * Current generated entries never define the activation source, so this
 * module remains inactive for them.
 *
 * SCOPE
 *
 * Older Liquid Glass YAML on Chromium only (Chrome, Edge, and the Android
 * companion app's webview). Firefox and Safari keep that YAML's plain blur;
 * see `blinkResolvesFilterRefs`.
 */
...
(() => {
  ...
  // Older Liquid Glass YAML defines this variable for the module to switch to.
  // Current generated entries never define it. Presence remains the activation
  // signal, with no theme-name matching.
  const SOURCE_VARIABLE = "--ha-glass-refraction-backdrop";

  // Displacement tuning published by compatible older YAML. Read rather than
  // duplicated, with no fallback literals: when these are absent, no
  // compatibility source is active and the module installs nothing.
  ...
  /*
   * Enable the override only while compatible older YAML defines its source
   * and tuning variables. Current generated entries never activate it.
   *
  ...
  const sync = () => {
    ...
    // Installed here rather than at startup, and only once compatible older
    // YAML is active: the filter's tuning comes from that YAML, and current
    // generated entries therefore never grow the extra element.
