...
export const createDropdownSurfaceController = ({
  ...
  const applyBlur = (dropdown) => {
    const ownedStyle = styled.get(dropdown);
    if (ownedStyle?.parentNode === dropdown.shadowRoot) return;
    if (ownedStyle) removeBlur(dropdown);
...
if (typeof document !== "undefined") {
  const controller = createDropdownSurfaceController({
    document,
    getComputedStyle,
    MutationObserver,
    customElements,
  ...
  controller.start();
  requestAnimationFrame(() => controller.sync());
  setTimeout(() => controller.sync(), 500);
}
